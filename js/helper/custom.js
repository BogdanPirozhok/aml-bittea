(function() {
    ...
    window.glossary = [
        ...
        'Данные успешно изменены | Data changed successfully',
        ...
        'Пароль должен содержать буквы латинского алфавита, хотя бы одну цифру и букву в верхнем регистре | The password must contain letters of the Latin alphabet, at least one number and an uppercase letter',
        ...
        'Продолжить | Proceed',
        'продолжить | proceed',
        'Вы действительно хотите удалить заявку? | Are you sure you want to delete the application?',
        'полная | complete',
        'Шаг | Step',
        ...
        'выполнено | performed',
        'стоимость | cost',
        'последнее обновление данных | last data update',
        'Телефон | Phone',
        'Редактировать профиль | Edit profile',
        'Обновить данные | Update the data',
        'Процедуры проверки: | Verification Procedures:',
        'Комментарий: | A comment:',
        'Создана | Created by',
        'новая заявка | new application',
        'Менеджер | Manager',
        'принял(a) заявку | accepted the application',
        'проведена | carried out',
        'проверка | check',
        'Проверка | Check',
        'Вы не заполнили обязательные поля | You did not fill in the required fields',
        'завершена | completed',
        'ФИО | Full name',
        'Комментарий | A comment',
        'Стоимость заявки | Application cost',
        'Редактировать | Edit',
        'Создать заявку | Make application',
        'Бета-версия! В данный момент эта функция недоступна, в скором времени мы закончим работу и все функции будут доступны | Beta version! This function is not available at the moment, we will finish work soon and all functions will be available',
        'у вас нет заявок | you have no applications',
        'Нажмите «Новая заявка», чтобы начать работу и пройти первую процедуру проверки. | Click «New Application» to start work and go through the first verification procedure.',
        'Бета-версия. Функция будет доступна после окончания работ. | Beta version. The function will be available after the end of work.'
    ];

    window.__ = function(str){
        let data = window.glossary,
            lang = {
        ...
        if ( lang.selected != 'ru' ) {
            for (var i = 0; i < data.length; i++) {
                if ( data[i].toLowerCase().indexOf(str.toLowerCase()) != -1 ) {
                    return data[i].split(' | ')[lang.items.indexOf( lang.selected )];
                }
            }
